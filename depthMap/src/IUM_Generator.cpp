...
void IUM_Generator::render()
{
	...
	OptixManager::instance().render(launchParams.size.x, launchParams.size.y, 1, pipeline, raygenRecordsBuffer, sbt);

	int dimension = launchParams.size.x * launchParams.size.y;

	// Scarica i risultati dalla GPU
    std::vector<vec3f> positions(dimension);
    std::vector<uint8_t> masks(dimension);
    
    positionsBuffer.download(positions.data(), dimension);
    masksBuffer.download(masks.data(), dimension);
